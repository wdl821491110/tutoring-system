...
async function doLogin() {
    ...
    try {
        ...
        updateUIByRole(); switchPage('dashboard'); updateUserDisplay();
...
function showRegisterForm() { $('#loginOverlay').style.display = 'none'; $('#registerOverlay').style.display = 'flex'; }
function hideRegisterForm() { $('#registerOverlay').style.display = 'none'; $('#loginOverlay').style.display = 'flex'; }

async function doRegister() {
    const u = $('#regUsername').value.trim(), p = $('#regPassword').value;
    if (!u || !p) { $('#regError').textContent = '请填写用户名和密码'; $('#regError').style.display = 'block'; return; }
    if (p.length < 6) { $('#regError').textContent = '密码至少6位'; $('#regError').style.display = 'block'; return; }
    try {
        const json = await api('/api/auth/register', { method: 'POST',
            body: JSON.stringify({ username: u, password: p, role: $('#regRole').value,
                real_name: $('#regRealName').value.trim(), phone: $('#regPhone').value.trim() }) });
        if (json.code !== 200) { $('#regError').textContent = json.message; $('#regError').style.display = 'block'; return; }
        toast(json.message, 'success'); hideRegisterForm();
    } catch (e) { $('#regError').textContent = '注册失败'; $('#regError').style.display = 'block'; }
}
    ...
    if (tok && usr) {
        ...
        api('/api/auth/me').then(res => {
            if (res.code === 200) {
                ...
                updateUIByRole(); switchPage('dashboard'); updateUserDisplay();
            } else { localStorage.removeItem('tutoring_token'); localStorage.removeItem('tutoring_user'); }
        }).catch(() => {});
    }
}
...
// ==================== 初始化 ====================
function init(){initNavigation();bindFilters();tryAutoLogin();}
